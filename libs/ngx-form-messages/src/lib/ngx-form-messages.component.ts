...
@Component({
  ...
  imports: [CommonModule, NgxFormMessageComponent, NgxLocalPipeModule],
...
export class NgxFormMessagesComponent implements AfterContentInit {
  ...
  public readonly cutomFormMessageComponentList$: Observable<
    ReadonlyArray<
      NgxFormMessageComponent & Required<Pick<NgxFormMessageComponent, "error">>
    >
  >;
  public readonly errors$: Observable<ValidationErrors | undefined>;
  public readonly isVisible$: Observable<boolean>;
  public readonly ngxFormMessageConfig$: Observable<
    Required<NgxFormMessageConfig>
  >;
  public readonly staticErrorList$: Observable<
    ReadonlyArray<keyof NgxFormMessageConfig>
  >;
  ...
  private readonly controlSource = new ReplaySubject<
  ...
  constructor(
    ...
    @Inject(NGX_FORM_MESSAGE_CONFIG)
    @Optional()
    ngxFormMessageConfigFactory: NgxFormMessageConfigFactory | undefined,
  ) {
    ...
    const externalNgxFormMessageConfig$ = this.getNgxFormMessagesConfig(
      ngxFormMessageConfigFactory,
    );

    this.ngxFormMessageConfig$ = externalNgxFormMessageConfig$.pipe(
      map((ngxFormMessageConfig) => {
        const initialNgxFormMessagConfig =
          this.getInitialNgxFormMessagConfig(ngxFormMessageConfig);

        return initialNgxFormMessagConfig;
      }),
    );

    this.cutomFormMessageComponentList$ = this.afterContentInitSource
      .asObservable()
      .pipe(
        switchMap(() => {
          return this.ngxFormMessageCustomList.changes.pipe(
            startWith(this.ngxFormMessageCustomList.toArray()),
          );
        }),
      );

    this.staticErrorList$ = this.ngxFormMessageConfig$.pipe(
      map(objectKeys),
      withLatestFrom(this.cutomFormMessageComponentList$),
      map(([staticErrorList, cutomFormMessageComponentList]) => {
        return staticErrorList.filter((staticError) => {
          return cutomFormMessageComponentList.every(
            (cutomFormMessageComponent) => {
              return cutomFormMessageComponent.error !== staticError;
            },
          );
        });
      }),
    ...
    this.isVisible$ = combineLatest([
      ...
      map(([whenCondition, isSubmitted, status]) => {
        return (whenCondition || isSubmitted) && status === "INVALID";
      }),
    );
  }
  ...
  public identifyNgxFormMessageComponent(
    _index: number,
    ngxFormMessageComponent: NgxFormMessageComponent &
      Required<Pick<NgxFormMessageComponent, "error">>,
  ): string | number {
    return ngxFormMessageComponent.error;
  }

  public identifyStaticError(
    _index: number,
    staticError: keyof NgxFormMessageConfig,
  ): string | number {
    return staticError;
  }
  ...
  private getNgxFormMessagesConfig(
    ...
    const ngxFormMessageConfig$ =
      typeof ngxFormMessageConfigFactory === "function"
        ? ngxFormMessageConfigFactory()
        : undefined;
